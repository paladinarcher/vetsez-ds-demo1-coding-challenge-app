class WeeklyStatusDetail < ApplicationRecord
  ...
  scope :project_task_names, -> (pt) {select(:timesheet_cell_task_name).where(timesheet_cell_project_title: pt).distinct.order(:timesheet_cell_task_name)}
end

# pt = 'TO 7 - VLER VHIE Agile Development Services_Option Period 2'
# tns = WeeklyStatusDetail.project_task_names(pt)
